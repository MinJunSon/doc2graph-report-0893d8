...
window.REPORT = {
  ...
  /* 결론(맨 위 요약) — 의도 → 핵심 지표 → 진행 상태. 중간보고 프레임.
   * lead/tail 은 강조(<b>·<i>) 위해 HTML 그대로 렌더(작성자 통제 콘텐츠).
   * 카드 숫자는 hero 지표라 수동 기재 — 연결요소(섬·최대%)·eval 은 graph-*.js stats 에 없음.
   * 갱신 시 여기만 고치면 됨. */
  summary: {
    lead:
      "<b>만들고 싶은 것</b> — 텍스트 조각을 검색하는 보통의 RAG 를 넘어, 사실들이 서로 연결돼 " +
      "<i>멀티홉으로 추론되는</i>, 우리 RAG 를 한 단계 끌어올릴 <b>‘쓸만한’ 지식그래프</b>.",
    cards: [
      { value: "79.2%",     label: "최대 연결요소", sub: "v0.5 21.6%" },
      { value: "23 / 3 / 0", label: "26문항 회귀",  sub: "FAIL 0" },
      { value: "1,591",     label: "관계 엣지",     sub: "v0.5 744" },
      { value: "165",       label: "섬(연결요소)",  sub: "v0.5 457" },
    ],
    tail:
      "<b>v0.6 은 그 중간 지점</b> — 흩어져 있던 섬 457개를 165개로 이어(질병→분류코드·청구권→시효 " +
      "같은 멀티홉이 한 그래프 안에서 풀린다) <i>연결성 뼈대</i>를 세운 단계. 계속 고도화 중인 중간보고입니다.",
  },
